...
async def create_post(post: PostCreate, db: DbSession):
    user = (await db.execute(select(models.User).where(models.User.id == post.user_id))).scalars().first()
    if not user:
...
async def update_post_full(post_id: int, post_data: PostCreate, db: DbSession):
    ...
    if post_data.user_id != post.user_id:
        user = (
            await db.execute(select(models.User).where(models.User.id == post_data.user_id))
        ).scalars().first()
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
